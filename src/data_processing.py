...
class RFMTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        snap = self.snapshot_date or df['TransactionStartTime'].max() + pd.Timedelta(days=1)
        rfm = (
...
def build_and_run_pipeline(input_path: str, output_path: str):
    ...
    cat_df  = CategoricalAggregator(['ProductCategory','ChannelId','PricingStrategy']).transform(df)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate feature file from raw transactions")
    parser.add_argument("--input",  required=True, help="Path to raw CSV (data/raw/data.csv)")
